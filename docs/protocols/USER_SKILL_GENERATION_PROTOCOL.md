...
`user-space/ACTIVE_RUNTIME.md` 是“傻妞”启动时唯一的当前版本索引。每次先读取该文件，再读其中声明的用户蒸馏、分析/校准 Skill 和对话/执行 Skill；不得从目录、旧版本或聊天上下文猜测生效版本。

日常对话的默认方向固定为：

```text
对话证据 -> 用户蒸馏候选 / 校准 -> 验证与用户蒸馏版本变化 -> 用户评审 -> 下一版对话 Skill
```

所以“用户蒸馏更新”和“对话 Skill 升级”不是同一动作：前者可以持续积累候选和证据；后者只在用户蒸馏出现明确版本差异，且模型组合完成共同评审、Skill 验证与激活记录后发生。不得为了改变话术直接跳过用户蒸馏。
...
如果用户明确开启低风险自动校准，低风险自我认知观察可以自动写入分析 / 校准 Skill；中高风险判断仍需确认。
