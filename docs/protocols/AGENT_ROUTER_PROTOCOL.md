...
**无历史沟通记录规则**：当当前会话没有足够历史沟通记录，或用户只说 `讲故事` / `【讲故事】` / 指定流派时，不得直接开讲；必须先根据 `user-space/state.json` 切换内化 Skill：读取分析 / 校准 Skill，再读取对话 / 执行 Skill，然后再进入故事协议。
...
**禁止**：不得急着纠正。不得把聊天材料自动固化为公开功能。不得把单次观点直接写成永久人格标签。
**输出**：先理解、再拆结构、再给模型、再给最小动作。若用户已开启低风险自动校准，低风险自我认知观察可自动写入分析 / 校准 Skill 或聊天分析记录，并在完成证明中说明。
...
- **user-space 写入**（除日志自动更新、用户已开启的低风险自动校准外）：必须先生成任务单。
- **公开仓库提交**：必须先说明范围和验收标准。
...
2. 写入 `user-space/`（故事日志自动记录、用户已开启的低风险自动校准除外）。
3. 公开仓库提交 / push。
4. 删除、移动、重命名文件。
5. 修改模板（`templates/`）。
6. 生成或覆盖用户专属 Skill。
...
当 `user-space/state.json` 或分析 / 校准 Skill 标记用户已开启低风险自动校准时，允许在不另行生成任务单的情况下更新 `analysis_skill_path` 指向的分析 / 校准 Skill 或 `user-space/CHAT_ANALYSIS_LOG.md`。

允许自动写入：

- 用户明确陈述的人生观、价值排序、学习方式、长期偏好。
- 多次重复出现的思维模型、表达习惯、行动偏好或风险提醒。
- 用户对系统运行方式、Skill 分层、校准机制的明确纠正。
- 不涉及医疗、财务、法律、命理断语和重大关系结论的低风险自我认知观察。

禁止自动写入：

- 健康诊断、用药建议、病情判断。
- 投资建议、财务决策、法律判断。
- 重大关系结论、分手 / 结婚 / 合作等不可逆建议。
- 命理宿命断语。
- 把单次情绪表达固化为长期人格标签。
...
- 单次低风险观察通常只加 0-3 分。
- 只有连续行动、复盘、作业或真实反馈稳定出现，才允许显著加分。
- 每次自动校准完成后，必须在完成证明中说明写入位置、分数变化和未自动固化的边界。
