...
- 已从 `D:\Dev\Inner-Capacity-Skill` 完整迁入 Obsidian；旧目录暂时保留为回退来源，未删除。
...
> [!note] 迁移完整性
> 已迁入公开内核、`user-space`、版本化对话 Skill、模型迭代资料和参考模型工作树；共 3,100 个文件。嵌套 `.git` 元数据与两个临时 PDF 提取文件未迁入，因为它们不是 Skill 内容。原目录仍保留，待未来独立确认后再清理。
